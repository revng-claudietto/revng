...
concept PTMLEmitter = //
  Emitter<EmitterT> and requires(EmitterT &Emitter, llvm::StringRef String) {
    // auto makeTagInitializer(llvm::StringRef String);
    {
...
/// What a PTMLStreamEmitter writes to its stream. Metadata is only meaningful
/// alongside tags, so the two are a single mode instead of two flags.
