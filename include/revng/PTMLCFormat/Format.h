...
/// Starts a run of source characters that map one-to-one onto consecutive PTML
/// bytes: the character at SourceOffset sits at PTMLOffset, and every following
/// character advances both offsets by one, until the next sync point. Sync
/// points are recorded only where this correspondence breaks (a tag, or an
/// escaped character), so there is no entry per character.
