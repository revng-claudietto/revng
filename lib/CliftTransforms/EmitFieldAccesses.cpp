...
struct EmitFieldAccessesPass
  ...
  void runOnOperation() override {
    ...
    // Phase 3: Apply all replacements and update statistics
    for (const auto &R : Replacements) {
      FieldAccesses += R.BestTraversal.TraversedFields.size();
      ArrayAccesses += R.BestTraversal.TraversedArrays.size();
