...
PointerArithmeticBuilder::computePointerArithmetic(ExpressionOpInterface
  ...
  // If the traversal failed, or produced a `PointerArithmetic` that is not in
  // canonical form, we cannot rewrite the expression, so we leave it as raw
  // pointer arithmetic instead of asserting.
  // TODO: currently negative `Stride`s fail the verify, we may need to support
  //       them in the future.
