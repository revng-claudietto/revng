...
FunctionSummaryOracle importImpl(llvm::Module &M,
  ...
  auto RegisterFilter = std::views::filter([SP = CSVInfo.spReg()](GV *CSV) {
    return CSV != nullptr && CSV != SP;
  });
  PrototypeImporter Importer{
    .Level = Level,
    .M = M,
    .ABICSVs = CSVInfo.abiRegisters() | RegisterFilter
...
FunctionSummaryOracle::importFullPrototypes(llvm::Module &M,
  ...
  revng_log(Log,
            ...
            "account");
  return importImpl<PrototypeImportLevel::Full>(M, CSVInfo, Binary);
}
...
FunctionSummaryOracle::importBasicPrototypeData(llvm::Module &M,
  ...
  revng_log(Log,
            ...
            "data");
  return importImpl<PrototypeImportLevel::Partial>(M, CSVInfo, Binary);
}
...
FunctionSummaryOracle::importWithoutPrototypes(llvm::Module &M,
  ...
  return importImpl<PrototypeImportLevel::None>(M, CSVInfo, Binary);
}
