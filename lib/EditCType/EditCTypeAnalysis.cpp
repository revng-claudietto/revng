...
llvm::Error EditCType::run(Model &Model,
  ...
  EditCTypeState State(Model.get());
  if (auto Error = prepareActionHelper(State, Configuration.LocationToEdit))
    return Error;

  auto [FilteredTypeHeader, Out] = setupTemporaryFile();
  emitFilteredHeader(*Out, *Model.get(), State.TypeToEdit);
  Out->close();
  ...
  return parseCompiledC(State,
                        FilteredTypeHeader.path().str(),
                        Configuration.CCode);
}
