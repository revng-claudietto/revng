...
public:
  InvokeIsolatedFunctionsImpl(const model::Binary &Binary,
    ...
    RootInfo(RootModule),
    PCH(ProgramCounterHandler::fromModule(Binary.Architecture(),
                                          &RootModule)) {
  ...
  BasicBlock *createInvokeReturnBlock() {
    ...
    BranchInst::Create(RootInfo.dispatcher(), InvokeReturnBlock);
  ...
  void run() {
    ...
    BasicBlock *UnexpectedPC = RootInfo.unexpectedPC();
