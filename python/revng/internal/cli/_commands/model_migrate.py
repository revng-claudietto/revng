...
def _read_input(input_: str | None) -> Dict:
    if not input_:
...
def _validate_schema(ctx: ClickContext, model: Dict) -> int:
    with NamedTemporaryFile("w") as file:
        yaml.dump(model, file)
        return ctx.try_run(["model-opt", "--verify", "-o", os.devnull, file.name])
...
def model_migrate(ctx: ClickContext, input_: str | None, in_place: bool, output: str | None) -> int:
    ...
    model = _read_input(input_)

    migrate(model)

    if _validate_schema(ctx, model) == 0:
