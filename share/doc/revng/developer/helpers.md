...
declare !revng.csua !114 !revng.csvaccess.offsets.load !118 !revng.csvaccess.offsets.store !120 !revng.tags !12 void @helper_write_eflags(ptr noundef, i64 noundef, i32 noundef) #0
```

The `!118` and `!120` are references to metadata nodes defined at the end of the module.
Resolving them reveals the actual CSV lists:

```bash
$ revng opt -strip-debug -S \
    "$ROOT/share/revng/libtcg-helpers-declarations-only-x86_64.bc" \
    | grep -E "^!(118|119|120|121) ="
!118 = !{i32 0, !119}
!119 = !{!"_state_0x2848"}
!120 = !{i32 0, !121}
!121 = !{!"_cc_src", !"_state_0x286c", !"_state_0x2848", !"_cc_op"}
```
...
19:
  ...
  %24 = and i64 %23, u0xffffffffffff0000
  %25 = shl i32 %22, 8
...
Compare this with the C source: the `udiv`/`urem` implement the division, `%_rax` is the accumulator, and the two `unreachable` blocks (labels 14 and 18) are where `raise_exception_ra` used to be (division-by-zero and quotient-overflow checks).
